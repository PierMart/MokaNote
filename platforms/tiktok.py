...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'log', 'tiktok.log')),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("tiktok")

def download_tiktok_audio(url, output_basename="tiktok_audio"):
    """
    Télécharge l'audio d'une vidéo TikTok à partir de l'URL fournie.
...
def handle_tiktok(url: str):
    audio_path = download_tiktok_audio(url, "tiktok_audio")
    ...
    for _ in range(20):
        ...
        return "Erreur : impossible de télécharger l'audio TikTok."
